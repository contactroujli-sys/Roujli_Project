...
export async function createRequest(userId, data) {
    ...
    if (data.serviceId !== undefined)
        createData.serviceId = data.serviceId;
    if (data.offerId !== undefined)
        createData.offerId = data.offerId;
    ...
    if (data.preferredDate !== undefined)
        createData.preferredDate = data.preferredDate;
...
export async function updateRequestStatus(id, ownerUserId, status, ownerNote) {
    const req = await prisma.request.findUnique({
        where: { id },
    ...
    if (req.business.ownerId !== ownerUserId)
        throw new Error("Only business owner can update request status");
